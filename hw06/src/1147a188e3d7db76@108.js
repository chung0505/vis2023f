...
function _selectedDataSet(Inputs){return(
Inputs.checkbox(["artist", "artistpublic"], {label: "Choose datasets", value: ["artist", "artistpublic"]})
)}

function _15(Plot,artist_columnKey,data,selectedDataSet){return(
Plot.plot({
  height: 600,
  ...
  marks: [
    Plot.barY(data.filter(d => selectedDataSet.includes(d.series)), Plot.stackY({ 
      x: "value",
      y: "count",
      fill: "series",
      title: d => `${d.series}\nvalue: ${d.value}\ncount: ${d.count}`
...
function _selectedDataSet1(Inputs){return(
Inputs.checkbox(["artist", "artistpublic"], {label: "Choose datasets", value: ["artist", "artistpublic"]})
)}

function _chart(data,selectedDataSet1,d3)
{
  // 定義邊界大小，以及圖形的寬度和高度
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("viewof selectedDataSet")).define("viewof selectedDataSet", ["Inputs"], _selectedDataSet);
  main.variable(observer("selectedDataSet")).define("selectedDataSet", ["Generators", "viewof selectedDataSet"], (G, _) => G.input(_));
  main.variable(observer()).define(["Plot","artist_columnKey","data","selectedDataSet"], _15);
  main.variable(observer()).define(["md"], _16);
  main.variable(observer("viewof selectedDataSet1")).define("viewof selectedDataSet1", ["Inputs"], _selectedDataSet1);
