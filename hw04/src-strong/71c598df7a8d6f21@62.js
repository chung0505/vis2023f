...
function _innerCircleQuestion(artist11){return(
Object.keys(artist11[0])[1]
)}

function _outerCircleQuestion(artist11){return(
Object.keys(artist11[0])[16]
)}

function _data(artist11,innerCircleQuestion,outerCircleQuestion,buildHierarchy)
{
  // 提取內外圈問題的答案
  var innerCircleAnswer = artist11.map(row => row[innerCircleQuestion]);
  var outerCircleAnswer = artist11.map(row => row[outerCircleQuestion]);
...
<h3>從上圖中，我們可以看出：
  <ul>
    <li>藝術工作者比較多人填問卷</li>
    <li>前三多的工作地點都採取減少包裝的方式達到減碳的功效</li>
    <li>校院前三多的工作地點採用相同的減碳方式但是占比順序不相同</li>
  </ul>
</h3>`
)}
...
d3
  ...
  .range(["#8E354A","#FF69B4","#DC9FB4","#FF1493","#4682B4","#F0F8FF"])
  .unknown("#BEBEBE")
)}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["artist-1@1.csv", {url: new URL("../artist@1.csv", import.meta.url), mimeType: "text/csv", toString}]
  ]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], _1);
  main.variable(observer("artist11")).define("artist11", ["__query","FileAttachment","invalidation"], _artist11);
  main.variable(observer("innerCircleQuestion")).define("innerCircleQuestion", ["artist11"], _innerCircleQuestion);
  main.variable(observer("outerCircleQuestion")).define("outerCircleQuestion", ["artist11"], _outerCircleQuestion);
  main.variable(observer("data")).define("data", ["artist11","innerCircleQuestion","outerCircleQuestion","buildHierarchy"], _data);
